...
const switchView = (newView, save = true, reRender = true) => {
    ...
    // Render kalender jika beralih ke view kalender dan belum diinisialisasi/dirender
    if (currentView === 'calendar' && calendarInstance && reRender) {
        // Beri sedikit waktu agar container terlihat sebelum render
        setTimeout(() => calendarInstance.render(), 50);
    }
    // Jika data sudah ada dan diminta render ulang (misal setelah filter), panggil filter
    else if (reRender && !initialLoad && allSchedules.length > 0) {
    ...
    try {
        ...
        if (cachedData) {
            ...
            if (Date.now() - timestamp < CACHE_EXPIRY_MS) {
                ...
                initialLoad = false;
                return; // Keluar dari fungsi jika cache valid
            } else {
...
const renderSchedules = (data) => {
    ...
    if (data.length === 0) {
        // Jangan tampilkan empty state jika view kalender aktif, biarkan kalender kosong
        if (currentView === 'grid') {
            showEmptyState();
        } else {
            hideEmptyState(); // Pastikan empty state tersembunyi di view kalender
        }
...
const renderCalendar = (data) => {
    ...
    // Mungkin perlu render ulang jika view kalender aktif
...
const hideLoading = () => {
    ...
    // Tampilkan elemen sesuai view saat ini HANYA jika tidak kosong
    if (elements.emptyState && !elements.emptyState.classList.contains('hidden')) {
        // Jika empty state tampil, sembunyikan grid dan kalender
        if(elements.scheduleGrid) elements.scheduleGrid.classList.add('hidden');
        if(elements.calendarContainer) elements.calendarContainer.classList.add('hidden');
    } else {
        // Jika tidak loading dan tidak empty, tampilkan elemen sesuai view
        if(elements.scheduleGrid) elements.scheduleGrid.classList.toggle('hidden', currentView !== 'grid');
        if(elements.calendarContainer) elements.calendarContainer.classList.toggle('hidden', currentView !== 'calendar');
    }
};
...
const hideEmptyState = () => {
    ...
    // Jangan langsung tampilkan grid/kalender di sini, biarkan hideLoading() yg atur
};
