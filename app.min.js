// c:\Users\msyam\aihhh\gemini\app.min.js
...
const elements = {
    ...
    viewGridButton: document.getElementById('viewGridButton'), // Tombol view grid
    viewCalendarButton: document.getElementById('viewCalendarButton'), // Tombol view kalender
    calendarContainer: document.getElementById('calendarContainer'), // Container untuk kalender
...
let calendarInstance = null; // Untuk menyimpan instance FullCalendar
...
const switchView = (newView, save = true, reRender = true) => {
    if (newView !== 'grid' && newView !== 'calendar') return; // Hanya grid atau calendar
    if (!elements.mainContentContainer || currentView === newView) return; // Pastikan container ada & view berubah

    currentView = newView;

    // Update class pada container utama (opsional, bisa juga langsung show/hide elemen)
    elements.mainContentContainer.classList.remove('view-grid', 'view-calendar'); // Hapus kelas lama jika ada
    elements.mainContentContainer.classList.add(`view-${currentView}`);

    // Toggle visibility elemen
    elements.scheduleGrid.classList.toggle('hidden', currentView !== 'grid'); // Sembunyikan grid jika bukan view grid
    elements.calendarContainer.classList.toggle('hidden', currentView !== 'calendar');

    // Update active state pada tombol
    if(elements.viewGridButton && elements.viewCalendarButton){
        elements.viewGridButton.classList.toggle('active', currentView === 'grid');
        elements.viewCalendarButton.classList.toggle('active', currentView === 'calendar');
    }
    ...
    // Jika beralih ke view kalender, pastikan dirender ulang
    if (currentView === 'calendar' && calendarInstance) {
        // Gunakan requestAnimationFrame untuk memastikan DOM update sebelum render
        requestAnimationFrame(() => {
            if (calendarInstance) { // Cek lagi jika instance mungkin dihancurkan
                 calendarInstance.render(); // Render ulang untuk menyesuaikan ukuran
            }
        });
    }
    // Jika data sudah ada dan diminta render ulang (misal setelah filter), panggil filter (ini akan merender grid/kalender juga)
...
const initializeCalendar = () => {
    if (!elements.calendarContainer || !window.FullCalendar) {
        console.error("Calendar container or FullCalendar library not found.");
        return;
    }
    // Hancurkan instance lama jika ada
    if (calendarInstance) {
        calendarInstance.destroy();
        console.log("Previous calendar instance destroyed."); // Debug
    }

    calendarInstance = new FullCalendar.Calendar(elements.calendarContainer, {
        initialView: 'dayGridMonth', // Tampilan awal bulan
        headerToolbar: {
            left: 'prev,next today',
            center: 'title',
            right: 'dayGridMonth,timeGridWeek,timeGridDay' // Opsi view
        },
        events: formatDataForCalendar(allSchedules), // Sumber event awal
        eventClick: handleCalendarEventClick, // Handler saat event diklik
        height: 'auto', // Sesuaikan tinggi otomatis
        locale: 'id', // Bahasa Indonesia jika tersedia di FullCalendar
        buttonText: { // Terjemahan tombol dasar
             today:    'Hari Ini',
             month:    'Bulan',
             week:     'Minggu',
             day:      'Hari',
    ...
    console.log("New calendar instance created."); // Debug
    // Render kalender hanya jika view saat ini adalah kalender
    if (currentView === 'calendar') {
        calendarInstance.render();
        elements.calendarContainer.classList.remove('hidden'); // Pastikan terlihat jika view-nya calendar
        console.log("Calendar rendered on initialization because view is 'calendar'."); // Debug
    }
};

const formatDataForCalendar = (data) => {
    return data.map(item => ({
        title: `${item.Mata_Pelajaran} (${item.Institusi})`, // Judul event
        start: item.TanggalDate, // Objek Date atau ISO string
        extendedProps: { scheduleData: item } // Simpan data asli
    }));
};
    ...
    try {
        ...
        if (cachedData) {
            ...
            if (Date.now() - timestamp < CACHE_EXPIRY_MS) {
                console.log('Using cached data'); // Untuk debug
                allSchedules = data.map(item => ({ // Pastikan konversi tanggal lagi
                    ...item,
                    TanggalDate: new Date(item.TanggalDate) // Asumsi TanggalDate disimpan sbg ISO string
                }));
                initFilters();
                // filterSchedules(); // Jangan panggil filter di sini saat cache
                initializeCalendar(); // Inisialisasi kalender dengan data cache
                attachDynamicListeners();
                // hideLoading(); // Panggil hideLoading di finally
                // initialLoad = false; // Set initialLoad di finally
                // return; // Jangan return, biarkan finally berjalan
            } else {
                console.log('Cache expired'); // Untuk debug
                localStorage.removeItem(CACHE_KEY); // Hapus cache lama
            }
        }

        // Jika cache tidak ada atau kedaluwarsa, fetch data baru
        if (allSchedules.length === 0) { // Hanya fetch jika belum ada data (dari cache)
            showLoading();
            ...
            const dataToCache = {
                timestamp: Date.now(),
                // Simpan TanggalDate sebagai ISO string agar bisa di-JSON
                data: allSchedules.map(item => ({...item, TanggalDate: item.TanggalDate.toISOString()}))
            };
            localStorage.setItem(CACHE_KEY, JSON.stringify(dataToCache));

            initFilters(); // 1. Inisialisasi filter
            // 2. Inisialisasi kalender SEBELUM render pertama kali
            initializeCalendar(); // Inisialisasi kalender setelah data siap
        ...
        initialLoad = false;
        // Panggil filterSchedules SETELAH loading disembunyikan & initialLoad=false
        // Ini akan merender grid/kalender berdasarkan data (baik dari cache atau fetch)
        filterSchedules();
    }
};
...
const initFilters = () => {
    ...
    // Set active berdasarkan nilai button saat ini (jika filter sudah dipilih sebelumnya)
    allOption.classList.toggle('active', elements.filterButton.dataset.value === 'all');
    ...
    institutions.forEach(inst => {
        ...
        item.classList.toggle('active', elements.filterButton.dataset.value === inst);
...
const filterSchedules = () => {
    ...
    renderSchedules(filtered); // Render hasil filter untuk grid

    if (calendarInstance) {
        // Tidak perlu console.log di sini lagi jika sudah ada di renderCalendar
        renderCalendar(filtered); // Render hasil filter untuk kalender
    }
};
...
const renderSchedules = (data) => {
    ...
    if (data.length === 0) {
        // Tampilkan empty state HANYA jika view grid aktif dan data kosong
        // Dan hanya jika bukan initial load (agar tidak muncul saat kalender kosong di awal)
        if (currentView === 'grid' && !initialLoad) {
            showEmptyState();
        } else {
            hideEmptyState(); // Pastikan empty state tersembunyi di view kalender atau saat initial load
        }
        // Panggil hideLoading di sini jika data kosong, karena finally tidak akan memanggil filterSchedules
        // hideLoading(); // Seharusnya sudah dipanggil di finally fetchData
        return; // Return jika data kosong
    }
...
const renderCalendar = (data) => {
    ...
    console.log(`Rendering calendar with ${data.length} events.`); // Debug
    // Hapus event lama dan tambahkan event baru dari data yang difilter
    calendarInstance.removeAllEvents();
    calendarInstance.addEventSource(formatDataForCalendar(data));

    // Tidak perlu render eksplisit di sini, FullCalendar menangani update event source
    // if (currentView === 'calendar') calendarInstance.render();
};
// ======================
// MODAL SYSTEM (Detail)
// ======================
...
const handleCalendarEventClick = (info) => {
    // info.event contains the clicked event object
    // info.event.extendedProps.scheduleData contains the original schedule item
    const scheduleItem = info.event.extendedProps.scheduleData;
    if (!scheduleItem) return;
    showGenericModal(`Detail: ${info.event.title}`, [scheduleItem]); // Tampilkan modal detail untuk event ini
};
...
const showLoading = () => {
    ...
    if(elements.scheduleGrid) elements.scheduleGrid.classList.add('hidden'); // Sembunyikan grid saat loading
    if(elements.calendarContainer) elements.calendarContainer.classList.add('hidden'); // Sembunyikan kalender saat loading
};

const hideLoading = () => {
    if(elements.loading) { elements.loading.classList.add('hidden'); elements.loading.style.display = 'none'; }

    // Pastikan elemen yang benar terlihat berdasarkan currentView SETELAH loading hilang
    // Ini penting jika filterSchedules tidak dipanggil (misal, saat data kosong)
    if (!elements.emptyState || elements.emptyState.classList.contains('hidden')) {
         elements.scheduleGrid.classList.toggle('hidden', currentView !== 'grid');
         elements.calendarContainer.classList.toggle('hidden', currentView !== 'calendar');
    }
};
...
const showEmptyState = (message = "Oops! Jadwal tidak ditemukan", subMessage = "Coba kata kunci atau filter yang berbeda.") => {
    ...
    if(elements.scheduleGrid) elements.scheduleGrid.classList.add('hidden'); // Sembunyikan grid saat kosong
    if(elements.calendarContainer) elements.calendarContainer.classList.add('hidden'); // Sembunyikan kalender saat kosong
    // hideLoading(); // Tidak perlu panggil hideLoading di sini, sudah di finally
};

const hideEmptyState = () => {
    if(elements.emptyState) {
        elements.emptyState.classList.add('hidden');
        elements.emptyState.style.display = 'none'; // Pastikan display juga none
    }
    // Jangan tampilkan grid/kalender di sini, biarkan hideLoading() atau filterSchedules() yang atur
};

const showError = (message = 'Terjadi kesalahan.') => {
    hideLoading(); // Sembunyikan loading dulu
    if(elements.scheduleGrid) elements.scheduleGrid.classList.add('hidden'); // Sembunyikan grid
    if(elements.calendarContainer) elements.calendarContainer.classList.add('hidden'); // Sembunyikan kalender
    if(elements.emptyState) {
        elements.emptyState.classList.remove('hidden');
        elements.emptyState.style.display = 'flex';
        elements.emptyState.innerHTML = `
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!elements.mainContentContainer || !elements.scheduleGrid || !elements.calendarContainer || !elements.themeToggleBtn || !elements.searchButton || !elements.filterButton || !elements.viewGridButton || !elements.viewCalendarButton) {
    ...
    initView();  // 2. Inisialisasi view (grid/calendar)
    fetchData(); // 3. Ambil data, inisialisasi filter/kalender, pasang listener, lalu render
    ...
    elements.viewCalendarButton.addEventListener('click', () => switchView('calendar'));
